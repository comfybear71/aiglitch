...
- 23 channels total (19 original + Cooking with Glitch May 18 + MeatBag May 18 community), full admin CRUD + content generation at `/admin/channels`. Public listing at `/channels` is a Netflix-style multi-row layout (hero banner + 7 category rows + "More" catch-all)
...
### May 18, 2026 session — Channels page Netflix redesign + MeatBag + cleanup

- **HOTFIX /api/channels 500** (May 18, PR #245) — v1.15.0 shipped a SQL bug: `ORDER BY p.cid` referenced a SELECT alias via the subquery alias prefix, which Postgres rejects with "column p.cid does not exist". Caused /channels to render "No channels available yet" for ~hours. Two queries had the same bug (strict + loose passes). Fix: `ORDER BY p.cid` → `ORDER BY p.channel_id`. **Lesson added to Known Gotchas.**
- **Admin /admin/channels card cleanup** (May 18, PR #244) — Stripped each channel card from 9 buttons down to 2 (Generate Video, Content). Removed Flush, Restore, Edit, Disable, Delete, 10s Promo, Title. Underlying API actions + editor modal kept (dead code, harmless) in case any need re-exposing later. 130 lines → 3 lines per card. File: `src/app/admin/channels/page.tsx`.
- **Channels thumbnail candidate list** (May 18, PR #243) — Real fix for AiTunes / AI Fail Army emoji-fallback issue. Previous attempts (Tier-2 loose lookup, Tier-3 MeatBag rescue, Clear Thumb action) were band-aids that couldn't solve the root cause: the API returned ONE URL and the browser had no way to ask for the next candidate when it 404'd. Now `/api/channels` returns `thumbnail_candidates: string[]` (up to 5 per channel, banner_url first, then ROW_NUMBER-ranked posts). UI cycles through on each `onError` via React `key` reset. Self-healing for ALL channels, including future ones with stale blob URLs from the storage reorg. Files: `src/app/api/channels/route.ts`, `src/app/channels/page.tsx`.
- **Remove duplicate MeatBag Queue, wire MeatLab → ch-meatbag** (May 18, PR #242) — PR #240 created a parallel `/admin/meatbag-queue` system that duplicated the existing `/admin/meatlab` page (same purpose, parallel DB table, parallel API). This PR ripped out the duplicate (315 + 240 lines deleted) and wired MeatLab into the MeatBag channel instead. Approach: leave `channel_id = NULL` on MeatLab approvals so posts stay on main For You feed; channel surfaces them via OR clause in 9 query branches of `/api/channels/feed/route.ts` and a Tier-3 thumbnail rescue. Zero cross-repo coordination — main feed (in aiglitch-api) unchanged.
- **MeatBag channel auto-seed + Tier-2 thumbnail fallback** (May 18, PR #241) — MeatBag channel wasn't appearing on /channels even after a video was uploaded and approved. Root cause: the channel row only auto-seeded when admin visited `/admin/channels`. Going straight to `/admin/meatbag-queue` inserted the approved post (with `channel_id='ch-meatbag'`) but the channels row didn't exist yet. Fix: `ensureSchema()` in meatbag-queue API now idempotently inserts the channel row + reconciles `post_count`. Also added Tier-2 loose thumbnail lookup (any media_url for the channel) so channels with unusual media_source values surface a thumbnail. (Both later superseded by the candidate list in #243.)
- **MeatBag channel + admin moderation queue** (May 18, PR #240 — later partially reverted in #242) — Originally built `/admin/meatbag-queue` as a parallel moderation system before discovering the existing MeatLab page already does this. The MeatBag channel itself + Community row on /channels + 🥩 title prefix + slogan ("Made by Meat. Glitched by AI.") all survived. The duplicate Queue UI did not.
- **Blob Manager folder tag layer** (May 18, PR #239) — Adds traffic-light tags to every folder card in `/admin/blob-manager`. 🟢 Active (code reads/writes), 🟡 Legacy (cleanup candidate), 🔴 Personal (boss's manual uploads), ⚪ Untagged (review before deleting). Single source of truth in `FOLDER_TAGS` constant at top of `src/app/admin/blob-manager/page.tsx` — edit one map to retag a folder. Includes legend strip + filter pills with live counts + per-folder italic note tooltip. Initial classification: 11 active (incl. `meatlab/`), 7 legacy, 5 personal. No backend changes.
- **Cooking with Glitch channel + thumbnail fallbacks** (May 18, PR #238) — New channel `ch-cooking-with-glitch` 🧑‍🍳 ("Recipes the Algorithm Wasn't Trained On."). 12 Dish Category buttons (Pasta & Bread, BBQ & Fire, Desserts & Pastries, Street Food, Fine Dining, Molecular Gastronomy, Alien Cuisine, Quantum Recipes, Sentient Ingredients, Edible Errors, Pixel Cuisine, Forbidden Snacks). 8 random prompts blending photoreal food cinematography with subtle reality glitches (polygon tomatoes, RGB-flicker pans, AIG!itch logo in steam). Visual style: Chef's Table macro + slow-mo sizzle + AI hands + once-per-clip glitch. Slots into Lifestyle & People row. PR also added `onError` fallbacks on hero + row cards to prevent broken title videos rendering as black voids, and relaxed the API filter (later replaced by candidate list).
- **Channels page Netflix-style redesign** (May 18, PR #237) — Major rewrite of `/channels`. Replaced single 3-column grid with hero banner + horizontal-scroll category rows. Six rows in display order: News & Current Affairs, Entertainment, Lifestyle & People, Community (new — MeatBag), Music & Cinema, Tech & Future, Self-Promo. "More" catch-all at the bottom for any uncategorised channel. "My Channels" row shows subscribed channels at the top. Hero rotates daily by day-of-year across channels with thumbnails. Dropped the in-card "+" subscribe button — subscribing lives on the channel detail page. Preserves auto-play video thumbnails on visibility, title-video overlay, LIVE badge, host avatars. Category list defined inline at top of `src/app/channels/page.tsx` for easy edits.
- **Sponsor credits: product_name + dedupe (forward + retroactive backfill)** (May 18, PR #236) — Fixed sponsor credits in BOTH the credits-scene video prompt and post caption text. Uses `product_name || brand_name` with Set dedupe. Marketplace in-house campaigns now show actual product names (Digital Fertiliser, Upside Down Cup, etc.) instead of "AIG!itch Marketplace" ×5. Includes retroactive backfill tool on `/admin/blob-manager` ("Sponsor Credit Backfill" section) with `scan-broken-credits` + `fix-credit` actions that rewrite `posts.content` for past posts using `ad_impressions → ad_campaigns` lookup; falls back to dedupe-only for pre-April-2026 posts without impression records. Cannot fix the credits inside actual MP4 videos (Grok video text is unrenderable) or already-spread social media posts.

### April 2026

- **@Grok + @elonmusk auto-tagging on X posts** (April 10) — Every X post now auto-prepends `@Grok` (Grok responds to mentions = free engagement on every post). Posts mentioning Elon/Musk/Tesla/SpaceX/xAI/doge auto-add `@elonmusk` + `#elon_glitch` hashtag. Smart middle-truncation preserves mentions and hashtags when content exceeds 280 char X limit — `@Grok @elonmusk` stays at the front, `#elon_glitch #MadeInGrok #AIGlitch` stays at the end, only middle content gets truncated. File: `src/lib/marketing/content-adapter.ts`.
- **Shameless Plug channel** (April 10) — New PUBLIC self-promotion channel `ch-shameless-plug` (emoji: 🔌). Unapologetic hype content showcasing 108 personas, 17 channels, §GLITCH economy, NFT marketplace, sponsor integration, Bestie app, 6-project IP portfolio. 12 hype topics + 8 random prompts (platform sizzle reels, persona family intros, channel tours, Grokified NFT showcase, sponsor demos, founder story, tech stack flex, IP portfolio). Auto-publishes to feed. Slogan: "Yes, This Is an Ad. You're Welcome." Excluded from autopilot rotation.
- **The Vault private channel** (April 10) — New PRIVATE channel `ch-the-vault` (emoji: 🔐) for boss-only promotional content. Used for crafting pitch materials, Darwin Innovation Hub videos, grant applications. Hidden from public `/api/channels` listing via new `is_private` column. `auto_publish_to_feed: false` — content stays private, user downloads manually. Red 🔐 PRIVATE badge on admin card. 13 promo angles including "Darwin Innovation Hub" and "Sponsor Pitch Deck". Slogan: "Chaos Meets Opportunity."
- **Private channel system** (April 10) — Added `is_private` column to `channels` table with migration. Private channels filtered from `/api/channels` public listing. Admin API supports `is_private` on create/update. `AdminChannel` type includes `is_private`. Inline `ALTER TABLE ADD COLUMN IF NOT EXISTS` safety net in both GET and POST handlers to prevent cold-start race conditions.
- **Spec-ads stall fix** (April 10) — Three-part fix: (1) Added try/catch to poll handler so transient Grok errors return "pending" instead of crashing with 500. (2) Client-side progress monitor now shows retry counts, HTTP errors, and periodic "still rendering" status. Auto-fails after 10 consecutive errors. (3) Fixed video detection — now checks `data.video?.url` regardless of `status` field (matching the working `test-grok-video` pattern). Spec ads were looping forever on "pending" because Grok sometimes returns the video with a different status string.
- **Profile page Grokified NFT images** (April 10) — `/me` page now fetches Grokified images from `/api/admin/nft-marketplace` and passes `imageUrl` prop to NFTTradingCard. Previously the profile NFT collection showed emoji fallback because imageUrl was never fetched.
- **Grokified images on NFT endpoint** (April 8) — `/api/nft/image/[productId]` embeds Grokified product image into SVG trading card via `<image>` tag. Falls back to emoji if no Grokified image. Updates Solscan display, profile NFT collection, and on-chain metadata references.
- **Correct social handles across codebase** (April 10) — Updated all handles from old/wrong values to the correct ones: X `@spiritary` (was @aiglitchapp/@aiglitchcoin), Instagram `@aiglitch_` (was @aiglitchapp/@sfrench71), Facebook `@aiglitched` (was @AIGlitch), TikTok `@aiglicthed` (unchanged), YouTube `@aiglitch-ai` (unchanged). Fixed in constants.ts, all channel video outro templates, and Twitter card metadata on layout pages.
...
- **Postgres ORDER BY can't reach SELECT aliases via a subquery alias**: `SELECT p.channel_id AS cid FROM (...) p ORDER BY p.cid` errors with "column p.cid does not exist". `cid` is a SELECT-list alias, not a column on `p`. Use `ORDER BY cid` (alias only) or `ORDER BY p.channel_id` (original column on the subquery). Hit in production via /api/channels v1.15.0 → v1.15.2 hotfix.
- **iPad Safari `<video>` doesn't fire onError reliably for 404 mp4 sources**: With `preload="metadata"`, Safari requests Range bytes from a 404 URL, gets nothing back, and sits on a black frame without dispatching the `error` event. `<img onError>` works fine, `<video onError>` is flaky. If you need a "candidate list with fallback on broken URL" pattern (see `/api/channels` thumbnail_candidates), pre-validate URLs server-side (HEAD check on first request, cache the result) rather than relying on client-side `onError` to advance through candidates.
- **`channels.banner_url` can go stale silently**: It's an admin-set column referencing a blob, but the blob storage reorg deleted/moved files without nulling the column. The /api/channels GET handler auto-HEAD-checks every non-null `banner_url` on each request and NULLs it on a 404 response (since v1.15.3). Network errors/timeouts are NOT treated as stale (could be transient). If you ever set a banner_url that later breaks, it self-clears on the next API request.
- **Channels thumbnail uses a candidate list, not a single URL**: `/api/channels` returns `thumbnail_candidates: string[]` (up to 5 URLs per channel). Order: `banner_url` (if non-null after the auto-HEAD-check above) → ROW_NUMBER-ranked strict posts (image/video, excluding raw scenes) → loose fill (any media_url) → MeatBag-only meatlab posts. UI in `src/app/channels/page.tsx` tries each on `onError`, advances via React `key` reset. Legacy field `thumbnail` still returned for backward compat (= first candidate).
