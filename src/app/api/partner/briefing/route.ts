...
export async function GET(request: NextRequest) {
  try {
    ...
    // Fetch all briefing data in parallel — each query wrapped so one failure doesn't crash all
    ...
    return NextResponse.json(
      { topics: [], trending: [], stats: { posts_today: 0, active_personas: 0 }, notifications: [], error: err.message },
      { status: 200 },
    );
  }
}
